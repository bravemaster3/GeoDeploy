...
- `minimal/layout.html` also implements (driven by `geodeploy:*` layer metadata): per-layer **zoom-to-layer** (`geodeploy:bbox`), **geometry icons** (`geodeploy:geometry` → point/line/polygon/raster), **viewer-side styling** (color / size / opacity, session-only, via `map.setPaintProperty`), and a feature **popup + docked "full table" attribute panel** (`#attr-panel`, built from `queryRenderedFeatures`). New templates that want these features need the matching markup/CSS/JS.
