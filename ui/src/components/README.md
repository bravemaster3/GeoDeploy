...
- `data/DiscoverModal.vue` — **import existing data**: two tabs (PostGIS tables / storage GeoTIFFs) from `/data/discover/*`, checkbox-select (already-imported disabled), imports as catalog rows (no copy), then refreshes the store.
