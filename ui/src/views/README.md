...
- `DataManager.vue` — lists vector + raster layers + **external sources** (WMS/XYZ/WFS) from the data store; opens upload modals, the "Connect source" modal (`AddSourceModal`), and the **"Import existing"** modal (`DiscoverModal`, registers data already in the connected DB/bucket). Polls happen via the store.
