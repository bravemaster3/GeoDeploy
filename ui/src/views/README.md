...
- `PortalEditor.vue` — the big one: left panel (a **folder tree** of layers via the recursive
  ...
  **Dashboard archetype (V-16, 2026-08-24):** a fourth Experience, `dashboard`. `isDashboard` mounts
  `components/portal/DashboardBuilder.vue` (v-model `dashboard`) in the Experience panel, fed by
  `dashboardLayers` (the portal's own vector + raster layers, with their `columns` — an external WMS
  has no attribute table to summarise and no COG to sample, so it is excluded, and binding a widget
  to a layer the portal does not publish would 404 for every visitor) and `dashboardPreset` (the
  chosen template's starting widget set). `dashboard` rides the save AND the preview payloads as
  `{}` when empty, which is what clears the column. The archetype defaults table gained a
  `dashboard` entry — **the same three-way parity mirror** as `resolveLayout`
  (`portal_generator` · `portal.js` · here).
...
widget bound to a layer the portal does not publish would 404 for every visitor. The archetype
defaults table gained its `dashboard` row — the same three-way parity mirror as `resolveLayout`.)
