...
- `templates.py` — `/templates` lists template folders from `/templates` that have both `template.json` and `layout.html`.
- `admin.py` — `/admin/health` (HTTP-pings Martin/TiTiler, inspects postgres/minio/redis containers), `/admin/reload-martin` (regenerates Martin config from all ready PostGIS layers — the manual recovery hook), `/admin/storage-stats`.
