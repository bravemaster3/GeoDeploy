...
- `admin.py` **update CHANNELS (2026-08-06 — issue #4, building on PR #1's target picker)**: `GET /admin/updates` reports FOUR targets, not one — `main` (`latest`/`behind`/`commits`), `latest_release` (newest published non-prerelease tag), `releases` (every published tag, so an operator can pin or roll back) and `branches` (unreleased work — how a feature branch is tried on a real instance). Releases come from BOTH GitHub endpoints because neither is sufficient: `/releases` has the title/date/prerelease flag but not the commit a tag points at, `/tags` has the sha; joined on tag name in `_load_releases`, which swallows failures so a rate-limited instance can still take a normal update. `channel`/`current_ref` come from `data/temp/deployed-ref` (`_deployed_ref`, re-read per request like `deployed-sha`) — a sha alone cannot say whether an instance follows a release or development, and a pinned instance must be judged against releases rather than nagged for being behind `main`. `_finalize_versions` re-derives all of that on the CACHED path too (the 10-min TTL is for GitHub's rate limit, never for our own version). `POST /admin/update {"target"}` (from PR #1) is unchanged — `_TARGET_RE`-validated, `main` normalised to `origin/main` so a stale local branch cannot win, `shlex.quote`d into the helper's command, and re-validated inside the script.
- `admin.py` **`GET /admin/credentials`** (owner-only, audited) reads `.env` FIRST, then the process environment, then `setup_config` — see `merge_credentials`, which is split out so the precedence is testable without a DB (`tests/test_connection_details_source.py`). It used to read the database row first, which is the one copy a RESTORE replaces with the snapshot's: the panel then showed another instance's MinIO keys, or the raw Fernet ciphertext when the two instances' `GEODEPLOY_SECRET_KEY`s differ (`decrypt_secret` returns its input on failure). `.env` is what every container is created from, so `.env` is what the instance runs on. Ciphertext is never displayed (`looks_encrypted` → `None`). Issue #2.
...
wrap TiTiler's TileJSON rather than proxying its own: TiTiler's `WMTSCapabilities.xml` self-
references `http://titiler:8000/…` — wrong host, no `/raster` prefix. The **TileMatrixSet is written
out in full** (25 levels, ScaleDenominator halving from 559082264.028717, TopLeftCorner, 2^z matrix
width/height) because QGIS will not zoom from a document that only names the matrix set. The tile
template rewrites `{z}/{x}/{y}` → `{TileMatrix}/{TileCol}/{TileRow}` and goes through `quoteattr`,
since an unescaped `&` from the query string makes the document malformed and QGIS rejects the whole
connection. Bounds fall back to `_titiler_bounds` for legacy rows, exactly like the TileJSON route.
Pinned by `api/tests/test_raster_wmts.py`. Share links now steer by tool: **WMTS for QGIS, TileJSON
for GeoLibre/MapLibre**, XYZ demoted to "no extent — prefer the other two".)
