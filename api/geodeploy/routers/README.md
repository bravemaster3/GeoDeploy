...
- `portals.py::export_portal_layer` — **public** `GET /portals/{slug}/export?layer_id=&bbox=&format=geojson|csv`: returns a portal vector layer clipped to the bbox (`ST_Intersects`), capped at 50k features, only for layers that belong to that portal. Used by the published portal's "select area & download" tool.
