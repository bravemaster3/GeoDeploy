...
#: The selection modes a map widget can offer. All three normalise to ONE geometry filter
#: downstream (a bbox IS a rectangular polygon), which is why they are a list of switches here
#: rather than three widget types.
MAP_TOOLS = ("click", "polygon", "bbox")


def _tools(value) -> list[str]:
    picked = [t for t in (value or MAP_TOOLS) if t in MAP_TOOLS]
    return picked or list(MAP_TOOLS)
...
def _normalize_source(widget_type: str, source: dict | None) -> dict | None:
    ...
    if spec["needs"] == "map":
        ...
        if ref and ref["layerType"] == "vector":
            ...
            if ref.get("field"):
                out["field"] = ref["field"]
            # Hit tolerance in degrees for point/line layers, where an exact click intersection
            # essentially never happens. 0 = polygons, which need none.
