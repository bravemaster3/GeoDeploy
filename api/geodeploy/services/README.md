...
- **Link previews (2026-08-18):** `portal_generator._social_meta(slug, title, description)` builds the Open Graph / Twitter card for a published portal, and `build_portal_bundle` injects it **after the `<head>` tag** rather than substituting a `{{…}}` placeholder — a portal renders through whichever `layout.html` its template ships, including one a user wrote, so a placeholder would exist only in the templates we control and a community template would silently lose its card. `og:image`/`og:url` must be ABSOLUTE and nothing here knows the instance's domain at publish time (bundles are also rebuilt by celery and by a restore, with no request to read a `Host` from), so both emit `__GEODEPLOY_ORIGIN__` and **nginx `sub_filter` substitutes the real origin per request** (`nginx/nginx.conf`, mirrored in `ui/nginx.conf` for the dashboard). The description is stripped of HTML + markdown and cut to 200 chars. Pinned by `tests/test_link_previews.py`.
...
## Last updated
2026-09-11b (**`line_opacity`**, the twin of `marker_opacity`: QGIS keeps a symbol's own opacity beside the colour AND an alpha inside the colour picker, and they multiply. The plugin was reading only the first, so a fill set fully transparent in the colour dialog published solid. Also **mixed content**: an `http://` tile source added to an instance served over `https` is upgraded when the provider answers over https, and refused with the reason when it cannot — a browser blocks it silently, so the layer would register and never draw. `secure_alternative` probes one real tile, substituting `{z}/{x}/{y}` first, because fetching a template verbatim asks for a file called `{z}`. Tests: `api/tests/test_external_kinds.py`.)

2026-09-11c (**a redirect to `http` is not `https`.** The check above followed redirects and asked only for a 200 with bytes in it, which the reported URL passes while ending on plain http: `www.google.cn/maps/vt?...` answers 302 to `http://www.google.com/...` however you ask it, so the naive upgrade stored an address that still could not draw. `_answers_securely` now requires every hop of the chain to be https, and `_redirected_origin` follows a pure change of HOST — path and query identical — so the source is stored at `https://www.google.com/maps/vt?...`, which serves the same bytes. A redirect that changes the path is NOT followed: a consent wall or a per-tile CDN URL is not a template.)
...
2026-08-17 (**contour lines, and one unpacking point for raster styles.** `algorithm=contours` joins
hillshade, with `increment`/`thickness`/`minz`/`maxz`. Two traps, both found against the live TiTiler
rather than reasoned about: its contours does not draw lines on a blank page — it colours the data
across `minz`–`maxz` with a built-in terrain ramp — and its defaults span the whole planet, so a
survey DEM covering 183–316 m renders as **a flat khaki rectangle**. The layer's own `rescale` is
therefore borrowed as that range. And `minz`/`maxz` are typed **int**: a fractional one returns
`422 int_from_float` for every tile, and stored stretches are routinely fractional
(`182.789993,315.959992`), so the borrowed range is floored/ceiled — widening the band rather than
clipping the extremes flat. `rescale` is NOT also sent as a query param, for the reason hillshade
already had. New `tile_url_from_style` replaces seven hand-written kwarg lists; two of them had
