...
- `titiler.py` — `get_tile_url(s3_key, colormap, rescale, algorithm)` → `/raster/cog/tiles/WebMercatorQuad/{z}/{x}/{y}?url=s3://...` (the `WebMercatorQuad` TileMatrixSet segment is **required** by the current TiTiler API). Supports `rescale` ("min,max" stretch) and `algorithm` (e.g. `hillshade`, single-band) in addition to `colormap_name`; algorithm and colormap are mutually exclusive. `get_tilejson_url` uses `/cog/WebMercatorQuad/tilejson.json`. `COLORMAPS` list.
