...
## Last updated
2026-09-11b (**`line_opacity`**, the twin of `marker_opacity`: QGIS keeps a symbol's own opacity beside the colour AND an alpha inside the colour picker, and they multiply. The plugin was reading only the first, so a fill set fully transparent in the colour dialog published solid. Also **mixed content**: an `http://` tile source added to an instance served over `https` is upgraded when the provider answers over https, and refused with the reason when it cannot — a browser blocks it silently, so the layer would register and never draw. `serves_over_https` probes one real tile, substituting `{z}/{x}/{y}` first, because fetching a template verbatim asks for a file called `{z}`. Tests: `api/tests/test_external_kinds.py`.)
